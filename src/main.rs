...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_url = env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite://data/claim-source-trail.db".into());
    if let Some(path) = database_url.strip_prefix("sqlite://").and_then(|path| PathBuf::from(path).parent().map(PathBuf::from)) {
    ...
    let dist_dir = env::var("DIST_DIR").map(PathBuf::from).unwrap_or_else(|_| PathBuf::from("dist"));
